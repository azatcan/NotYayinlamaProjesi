...
namespace ÜNY.BackofficeUI.Areas.Admin.Controllers
{
    ...
    public class AdminExamGradeController : Controller
    {
        ...
        {
            var exams = await _client.GetAsync<List<ExamDTO>>(DefaultClientEndpoint.Exam.GetExams);
            return View(exams);
        }
        ...
        [HttpPost("AddGrade")]
        public async Task<IActionResult> AddGrade(AdminExamGradeDTO model)
        {
            ...
            var response = await _client.PostAsync<AdminExamGradeDTO, ContactResponse>(DefaultClientEndpoint.AdminExamGrade.Add, model);
